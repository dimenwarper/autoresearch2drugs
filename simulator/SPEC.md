...
The benchmark is not RL-first. It should expose a rich state/action loop, structured observability artifacts, and multiple reported outcome metrics. The primary north-star score remains:
...
* Derived observability artifacts: append-only event records, timeline frames, and detailed report blobs generated from observable state transitions
...
### 8.1 Runtime observability goals

The simulator must emit structured runtime artifacts so a separate utility can later replay and visualize a run as a timelapse.

These artifacts are not part of the agent observation surface. They are an out-of-band observability layer for debugging, analysis, and visualization.

Observability requirements:

* every accepted state-changing action must emit at least one artifact record
* every rejected action must emit a rejection record with blocking reasons
* every scheduled-work completion must emit a completion record
* every stage transition, gate recomputation, and terminal condition must be reconstructable from emitted artifacts
* artifact emission must not change simulator state, action legality, or rewards

### 8.2 Artifact emission contract

For each run, the simulator should write an artifact bundle to a run-scoped directory:

```text
artifacts/<run_id>/
```

At minimum, the runtime should expose a non-agent helper such as:

```python
get_artifact_manifest() -> RunArtifactManifest
```

This helper is for debugging and replay utilities, not for the acting agent.

Artifact emission must be:

* deterministic given the same simulator version, seed, scenario preset, and action sequence
* append-only for streaming-friendly consumers
* observable-only by default
* versioned so later visualization utilities can reject incompatible schemas cleanly

### 8.3 Required artifact files

Each run must emit at least these files:

```text
artifacts/<run_id>/run_manifest.json
artifacts/<run_id>/event_log.ndjson
artifacts/<run_id>/timeline_frames.ndjson
artifacts/<run_id>/reports/studies/<study_id>.json
artifacts/<run_id>/reports/trials/<trial_result_id>.json
artifacts/<run_id>/reports/regulatory/<note_id>.json
artifacts/<run_id>/final_summary.json
```

`run_manifest.json` must include:

* `run_id`
* simulator/spec version
* RNG seed
* scenario preset
* initial portfolio config
* artifact schema version
* file paths or relative file refs for all emitted artifacts

### 8.4 Event log schema

`event_log.ndjson` is the canonical append-only replay stream.
...
Supported `event_type` values should include at least:
...
### 8.5 Timeline frame schema

`timeline_frames.ndjson` is optimized for later timelapse visualization.

Emit one full frame after:

* every successful state-changing instant action
* every completed scheduled work item
* every terminal event

Read-only actions do not need frames.
...
TimelineFrame = {
    ...
    "program_snapshots": list[dict],
    "visible_opportunities": list[dict],
    "active_work": list[dict],
    "recent_event_refs": list[int],
}
```

The frame payload must be sufficient for a visualization utility to reconstruct:

* portfolio cash over time
* which programs exist at each point
* each program's stage and operating status
* which work items are active
* which opportunities remain available
* major observations and completions

### 8.6 Detailed report artifacts

Detailed study, trial, and regulatory outputs should be emitted as standalone JSON blobs with stable IDs and cross-referenced from both the event log and timeline frames.

At minimum:

* each `StudySummary` should map to one study report artifact
* each `TrialResultSummary` should map to one trial report artifact
* each `RegulatoryNote` should map to one regulatory report artifact

These report artifacts may include richer text than the timeline frames, but they must still exclude hidden truth by default.

### 8.7 Hidden-state observability policy

By default, emitted artifacts must contain observable information only.

If a debug mode is implemented, hidden-state artifacts must:

* be disabled by default
* be written to a clearly separate debug-only location
* never be returned by agent-facing actions
* never be required by the timelapse utility

### 8.8 Final summary artifact

`final_summary.json` should contain:

* terminal portfolio state
* all reported metrics
* program terminal summaries
* artifact counts
* run end reason
* final `event_index`
* final `frame_index`
...
* `observability.py`: event emission, frame generation, artifact manifests, report serialization
...
* a versioned artifact schema for replay and timelapse tools
* a non-agent runtime helper to retrieve the emitted artifact manifest for the current run
...
5. logs and structured artifacts showing actions, event completions, observations, gate changes, and stage transitions
...
7. timelapse-friendly run artifacts including a manifest, event log, timeline frames, and detailed study/trial/regulatory report blobs
